...
"""
    ...
    with tf.device(
        tf.train.replica_device_setter(1, worker_device=worker_device)):
      with tf.variable_scope("global"):
        logger.info('global env.observation_space.shape: {}'.format(
            env.observation_space.shape))
        logger.info('global env.action_space.n: {}'.format(env.action_space.n))
    ...
    with tf.device(worker_device):
      with tf.variable_scope("local"):
        logger.info('local env.observation_space.shape: {}'.format(
            env.observation_space.shape))
        logger.info('local env.action_space.n: {}'.format(env.action_space.n))
        self.local_network = pi = LSTMPolicy(env.observation_space.shape,
                                             env.action_space.n)
      ...
      pi_loss = - \
          tf.reduce_sum(tf.reduce_sum(log_prob_tf * self.ac, [1]) * self.adv)
      ...
      logger.info('pi.var_list: {}'.format([tf.shape(x) for x in pi.var_list]))
      logger.info('self.network..var_list: {}'.format(
          [tf.shape(x) for x in self.network.var_list]))
      self.sync = tf.group(*[
